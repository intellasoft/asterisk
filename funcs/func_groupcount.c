...
/*** DOCUMENTATION
	...
	<function name="GROUP_MATCH_CHANNEL_LIST" language="en_US">
		...
		<description>
			...
			<example title="Group SubString Match">
				Find groups containing the string 'foo'
				On Channel SIP/1234-0000001:
					Set(GROUP()=groupName)
				On Channel SIP/1234-0000003:
					Set(GROUP()=foobarbaz)
				On Channel SIP/1234-0000004:
					Set(GROUP()=foobarbill)
				On Channel SIP/1234-0000005:
					Set(GROUP()=somethingelse)
				On any Channel:
					Assuming that channels still exist that have the above associated GROUP() assignents
					Set(channels_found=${GROUP_CHANNEL_LIST(.*foo.*)})
				Given the above:
					This will find the channels that are in groups 'foobarbaz' and 'foobarbill' since they both match .*foo.* regex against the group name
                                        Variable channels_found will be set to: SIP/1234-0000003,SIP/1234-0000004
			</example>
			<example title="Category SubString Match">
				Find groups containing category name of 'bar'
				...
				On any Channel:
					Assuming that channels still exist that have the above associated GROUP() assignents
					Set(channels_found=${GROUP_CHANNEL_LIST(@.*bar.*)})
				...
				Given the above:
					This will find the channels that are in groups 'foo@barbaz' and 'foo@barbill' since they both match .*bar.* regex against the category name
                                        Variable channels_found will be set to: SIP/1234-0000002,SIP/1234-0000003
			</example>
			<example title="Group and Category SubString Match">
				Find groups containing 'foo' and category name of 'bar'
				...
				On any Channel:
					Assuming that channels still exist that have the above associated GROUP() assignents
					Set(channels_found=${GROUP_CHANNEL_LIST(.*foo.*@.*bar.*)})
		...
		<see-also>
			<ref type="function">GROUP</ref>
			<ref type="function">GROUP_MATCH_LIST</ref>
		</see-also>
	</function>
	...
	<function name="GROUP_VAR" language="en_US">
		...
		<description>
			...
			<example title="Set and Read Group and Group Variables">
 			        On Channel 1
				Set(GROUP()=foo)
...
static struct ast_custom_function group_match_channel_list_function = {
	...
	.syntax = "GROUP_MATCH_CHANNEL_LIST(group[@category])",
